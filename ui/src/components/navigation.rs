...
fn NavItem(href: &'static str, icon: &'static str, children: Children) -> impl IntoView {
    ...
    let link_class = Signal::derive(move || format!(
    ...
    let icon_class = Signal::derive(move || format!(
        "h-6 w-6 shrink-0 {}",
        if is_active.get() {
            "text-blue-600"
        } else {
...
#[component]
fn Icon(name: &'static str, #[prop(into, optional)] class: Signal<String>) -> impl IntoView {
    ...
    view! {
        <svg class=move || class.get() fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
            {svg_content}
        </svg>
    }
}
